...
#  beautiful soup to open html file
    file = open(setwd+'souped_html_txt_files/'+cci_file_name_list[calendar_ingestion_count], 'r')
    file_read = file.read()
...
# some errors occur so as a backup we force first race and then remove duplicates.
    try:
        ...
        file = open(setwd+'souped_html_txt_files/'+cci_file_name_list[0], 'r')
        file_read = file.read()
        file_soup = BeautifulSoup(file_read, "html.parser")
        file_soup_part2 = file_soup.find_all('tbody')[-1]
...
# beautiful soup to open html file
    file = open(setwd+'souped_html_txt_files/'+cci_file_name_list[calendar_stage_profiles_ingestion_count], 'r')
    file_read = file.read()
    file_soup = BeautifulSoup(file_read, "html.parser")
...
first_cycling_calendar_df['paralympics_race_id'] = None

first_cycling_calendar_df = first_cycling_calendar_df[['season', 'gender', 'category','first_cycling_race_id','paralympics_race_id', 'race_name',
       'race_nationality', 'uci_race_classification', 'stage_race_boolean',
       'start_date', 'end_date', 'stage_number', 'distance', 'route',
       'stage_profile']]

# remove rest days from calendar_df

first_cycling_calendar_df = first_cycling_calendar_df.loc[first_cycling_calendar_df['stage_number'].notnull()]
